...
export default function Dashboard() {
  ...
  const [searchMode, setSearchMode]     = useState<"auto" | "keyword" | "url" | "1688">("auto");
  const [kwInput, setKwInput]           = useState("");
  const [urlInput, setUrlInput]         = useState("");
  const [storeUrlInput, setStoreUrlInput] = useState("");
  const [keyword1688, setKeyword1688]   = useState("");
  ...
  useEffect(() => {
    ...
    fetch(`/api/ebay/stores?userId=${user.uid}`)
      .then(r => r.json())
      .then(d => {
        if (d.stores?.length) { setStores(d.stores); setSelectedStoreId(d.stores[0].id); }
      });
  ...
  useEffect(() => { setLastDoc(null); setHasMore(false); setAllProducts([]); }, [user, selectedStoreId, activeTab]);
  ...
  // ── Publish ───────────────────────────────────────────────────────────────────
  const openPublishModal = (product: QueueProduct) => { if (!requireStore()) return; setPublishTarget(product); };
  ...
  const handlePublishConfirm = async (productId: string, storeIds: string[], stock: number) => {
    ...
    if (errors.length) { toast("❌ " + errors.join(" | "), "err"); throw new Error(errors.join(", ")); }
  ...
  const handlePublishAll = async () => {
    ...
    const ids = allProducts.filter(p => p.status === "approved").map(p => p.id);
  ...
  const handleRejectAll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) { toast(`❌ Error: ${data.error}`, "err"); }
      else { toast(`🗑 ${data.rejected} productos rechazados`, "ok"); setAllProducts(p => p.filter(x => x.status !== targetStatus)); }
    } catch { toast("❌ Error rechazando productos", "err"); }
    finally { setRejectingAll(false); }
  };
  ...
  const handleSearch = async (isAuto = false, startIndex = 0) => {
    ...
    try {
      if (isAuto) {
        ...
        if (sr.status === 401) {
          const errData = await sr.json();
          if (errData.error === "TOKEN_EXPIRED") { setTokenExpiredStore(selectedStoreId); toast("⚠️ eBay token expired — reconnect your store", "err"); }
        } else { toast("✅ Search complete", "ok"); }
      } else {
        await handleImport(); return;
      }
    } finally {
      setSearching(false); setPaused(false); pauseRef.current = false; setSearchProgress(null);
  ...
  const handleForcePublish = async (product: QueueProduct) => {
    ...
    const res  = await fetch("/api/ebay/publish", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ productId: product.id, storeId: selectedStoreId, userId: uid, forceVariations: true }),
    });
    const data = await res.json();
  ...
  const handleImportStore = async () => {
    ...
    setImportingStore(true); setImportProgress(null);
    try {
  ...
  const handleSearch1688 = async () => {
    if (!requireStore()) return;
    if (!keyword1688.trim()) { toast("Escribe un keyword primero", "err"); return; }
    setSearching(true);
    try {
      const res  = await fetch("/api/1688/search", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ keyword: keyword1688.trim(), userId: uid, storeId: selectedStoreId, markupPct: 40 }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      toast(`✅ ${data.added} productos de 1688 añadidos a Pending`, "ok");
  ...
  const handleRateLimits = async () => {
    if (!requireStore()) return;
    setLoadingRateLimits(true); setShowRateLimits(true);
    try {
  ...
  const handleImportSeller = async (userUrl: string, category: string) => {
    ...
    try {
      ...
      setActiveTab("approved"); setShowSellers(false);
    } finally { setImportingStore(false); }
  };
  ...
  const SidebarItem = ({ icon, label, badge, active, onClick }: {
    ...
    <button onClick={onClick} style={{
      ...
      {badge !== undefined && (
        <span style={{ fontSize: "0.68rem", fontWeight: 700, minWidth: 18, textAlign: "center",
  ...
  const SidebarLabel = ({ text }: { text: string }) => (
    <div style={{ padding: "0.6rem 0.85rem 0.2rem 1.25rem", fontSize: "0.65rem", fontWeight: 700,
      color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.09em" }}>
      {text}
    </div>
  );
    ...
    <div className="app-shell">
      ...
      <header className="app-header">
        ...
        <div style={{ display: "flex", gap: "0.35rem", flex: 1, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {stores.length === 0 && <span style={{ fontSize: "0.78rem", color: "var(--text3)" }}>Sin tiendas — crea una en My Stores</span>}
        </div>
      ...
      <div className="app-body">
        ...
        <nav className="app-sidebar" style={{ padding: "0.5rem 0", overflowY: "auto" }}>
          ...
          <SidebarItem icon="🛍" label="eBay Search"
            active={!showSellers && ["auto","keyword","url"].includes(searchMode)}
            onClick={() => { setShowSellers(false); if (searchMode === "1688") setSearchMode("auto"); }} />
          <SidebarItem icon="🇨🇳" label="1688"
            active={!showSellers && searchMode === "1688"}
            onClick={() => { setShowSellers(false); setSearchMode("1688"); }} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem 1rem",
              ...
              <div style={{ flex: 1 }}>
                ...
                <span style={{ color: "var(--text2)", marginLeft: 8 }}>
                  ...
                  {" "}({savedSearchState.keywordIndex}/{savedSearchState.total}) · {new Date(savedSearchState.savedAt).toLocaleTimeString()}
                </span>
              </div>
              <button onClick={() => handleSearch(true, savedSearchState.keywordIndex)}
                style={{ padding: "0.38rem 0.9rem", background: "var(--blue)", color: "#fff", border: "none",
                  borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.8rem", cursor: "pointer", whiteSpace: "nowrap" }}>
                ▶ Resume
              </button>
              <button onClick={() => { if (user) clearSearchState(user.uid); setSavedSearchState(null); }}
            ...
            {searchMode === "1688" ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                  ...
                  <strong style={{ fontSize: "0.9rem" }}>Buscar en 1688</strong>
                  <span style={{ fontSize: "0.72rem", color: "var(--text3)", marginLeft: "0.25rem" }}>vía Oxylabs · CNY → USD automático</span>
                </div>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <input value={keyword1688} onChange={e => setKeyword1688(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && !searching && handleSearch1688()}
                    placeholder="ej: organizador escritorio, soporte telefono, bolso mujer..."
                    style={{ flex: 1, padding: "0.5rem 0.8rem", background: "var(--bg3)",
                      border: "1px solid var(--border2)", borderRadius: "var(--radius-sm)",
                      color: "var(--text)", fontSize: "0.85rem", outline: "none" }} />
                  <button onClick={handleSearch1688} disabled={searching || !keyword1688.trim()}
                    style={{ padding: "0.5rem 1.2rem", background: "#e4393c", color: "#fff",
                      border: "none", borderRadius: "var(--radius-sm)", fontWeight: 700,
                      fontSize: "0.83rem", cursor: "pointer", opacity: (searching || !keyword1688.trim()) ? 0.6 : 1 }}>
                    {searching ? "⏳ Buscando..." : "🔍 Buscar"}
                  </button>
                </div>
                <div style={{ fontSize: "0.71rem", color: "var(--text3)" }}>
                  Los productos van a <strong>Pending</strong> para revisión manual · markup por defecto: 40%
                </div>
              </div>
            ) : (
              <>
                ...
                <div style={{ display: "flex", gap: "0.4rem", marginBottom: "0.85rem" }}>
                  {(["auto", "keyword", "url"] as const).map(m => (
                    <button key={m} onClick={() => setSearchMode(m)} style={{
                      padding: "0.28rem 0.85rem", borderRadius: 99, border: "1px solid",
                ...
                {searchMode === "auto" && (
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    ...
                    {stores.length > 0 && !stores.some(s => s.id === selectedStoreId && s.connected) && (
                      <span style={{ fontSize: "0.75rem", color: "var(--amber)" }}>⚠ Connect the store first</span>
                    )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                      <input value={storeUrlInput} onChange={e => setStoreUrlInput(e.target.value)}
                        placeholder="eBay store URL — ebay.com/str/... o username"
                        style={{ flex: 1, padding: "0.5rem 0.8rem", background: "var(--bg3)",
                          border: "1px solid var(--border2)", borderRadius: "var(--radius-sm)",
                    ...
                    <div style={{ fontSize: "0.71rem", color: "var(--text3)" }}>
                      Filtros: precio ≥ $15 · mínimo 3 ventas · sin IP violations · hasta 5,000 listings → pending
                    </div>
                    {importingStore && (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", minHeight: 36 }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                {(activeTab === "pending" || activeTab === "approved" || activeTab === "failed") && stats[activeTab] > 0 && (
                  <button onClick={() => setShowRejectAllConfirm(true)} disabled={rejectingAll}
                    style={{ padding: "0.4rem 0.9rem", background: "transparent", color: "var(--red)",
                      border: "1px solid var(--red)", borderRadius: "var(--radius-sm)", fontWeight: 600,
                      fontSize: "0.8rem", cursor: "pointer", opacity: rejectingAll ? 0.6 : 1 }}>
                ...
                {activeTab === "approved" && stats.approved > 0 && (
                  <>
                    ...
                    <button onClick={handlePublishAll} disabled={publishingAll}
                      style={{ padding: "0.4rem 1rem", background: "var(--green)", color: "#fff", border: "none",
                        borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.8rem",
                        cursor: "pointer", opacity: publishingAll ? 0.6 : 1 }}>
                ...
                {activeTab === "published" && stats.published > 0 && (
                  <>
                    ...
                    <button onClick={handleClean} disabled={cleaning}
                      style={{ padding: "0.4rem 0.9rem", background: "var(--red)", color: "#fff", border: "none",
                        borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.8rem", cursor: "pointer", opacity: cleaning ? 0.6 : 1 }}>
                      {cleaning ? "Revisando..." : "🧹 Limpiar"}
                    </button>
                    <button onClick={handlePromoteAll} disabled={promoting}
                      style={{ padding: "0.4rem 0.9rem", background: "var(--purple)", color: "#fff", border: "none",
                        borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.8rem", cursor: "pointer", opacity: promoting ? 0.6 : 1 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", alignItems: "center" }}>
                <button onClick={() => handleScanCategory(undefined)} disabled={!!scanningCategory}
                  style={{ padding: "0.38rem 0.9rem", background: "var(--purple)", color: "#fff", border: "none",
                    borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.78rem",
                    cursor: "pointer", opacity: scanningCategory ? 0.5 : 1 }}>
                  {scanningCategory ? "⏳ Scanning..." : "🔎 Scan for CN sellers"}
                </button>
                {scanningCategory && <span style={{ fontSize: "0.75rem", color: "var(--text3)" }}>Searching categories via Browse API...</span>}
                <button onClick={() => setEditingRange(r => !r)}
                  style={{ padding: "0.38rem 0.75rem", background: "transparent", color: "var(--text2)",
                    border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "0.78rem",
              ...
              {editingRange && (
                <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)",
                  padding: "0.75rem 1rem", display: "flex", flexWrap: "wrap", gap: "0.75rem", alignItems: "flex-end" }}>
                  {[["Min listings", "min", 1, listingRange.max - 1], ["Max listings", "max", listingRange.min + 1, undefined]].map(([label, key, min]) => (
                    <div key={key as string}>
                      <div style={{ fontSize: "0.68rem", color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 4 }}>{label}</div>
                      <input type="number" min={min as number}
                        value={listingRange[key as "min"|"max"]}
                        onChange={e => setListingRange(r => ({ ...r, [key as string]: Math.max(min as number, parseInt(e.target.value) || min as number) }))}
                        style={{ width: 90, background: "var(--bg1)", border: "1px solid var(--border)", borderRadius: 6,
                          color: "var(--text1)", padding: "0.3rem 0.5rem", fontSize: "0.85rem", outline: "none" }} />
                    </div>
                  ))}
                  <button onClick={async () => {
                    ...
                    toast(`✅ Rango: ${listingRange.min.toLocaleString()}–${listingRange.max.toLocaleString()}`, "ok");
                  }} style={{ padding: "0.38rem 0.9rem", background: "var(--green)", color: "#fff",
                    border: "none", borderRadius: "var(--radius-sm)", fontWeight: 600, fontSize: "0.78rem", cursor: "pointer" }}>
                    💾 Guardar
                  </button>
                </div>
              )}
              ...
              {savedSellers.length === 0 && !scanningCategory && (
                <div style={{ textAlign: "center", color: "var(--text3)", padding: "2rem", fontSize: "0.85rem", lineHeight: 1.6 }}>
                  Sin vendedores guardados — presiona <strong>Scan for CN sellers</strong>
                </div>
              )}
              {savedSellers.length > 0 && (
                <div style={{ fontSize: "0.72rem", color: "var(--text3)" }}>
                  {savedSellers.length} vendedores — ordenados por score
                </div>
              )}
                      ...
                      {sellers.map(s => (
                        <div key={s.username} style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "0.65rem 0.9rem", display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                            ...
                            {s.uniqueQueries ? <span style={{ fontSize: "0.72rem", background: "rgba(168,85,247,0.1)", color: "var(--purple)", border: "1px solid rgba(168,85,247,0.2)", borderRadius: 99, padding: "0.15rem 0.55rem" }}>🎯 {s.uniqueQueries} nichos</span> : null}
                            {s.appearances   ? <span style={{ fontSize: "0.72rem", background: "rgba(16,185,129,0.1)",  color: "var(--green)",  border: "1px solid rgba(16,185,129,0.2)",  borderRadius: 99, padding: "0.15rem 0.55rem" }}>👁 {s.appearances}</span>         : null}
                            <button onClick={() => handleDeleteSeller(s.username)}
                              style={{ fontSize: "0.72rem", padding: "0.2rem 0.5rem", background: "transparent", color: "var(--red)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", cursor: "pointer" }}>✕</button>
                          </div>
                          {s.sampleTitles?.slice(0, 2).map((t, i) => (
                            <div key={i} style={{ fontSize: "0.72rem", color: "var(--text3)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>· {t}</div>
                          ))}
                          <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.15rem" }}>
                            <button onClick={() => { navigator.clipboard.writeText(s.storeUrl); toast("📋 Link copiado", "ok"); }}
                            ...
                            <span style={{ fontSize: "0.7rem", color: "var(--text3)", alignSelf: "center" }}>Pega en Import Store</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1rem" }}>
              {loading ? (
                ...
                <div style={{ gridColumn: "1/-1", display: "flex", flexDirection: "column", alignItems: "center", padding: "4rem", gap: "0.75rem" }}>
                  <span style={{ fontSize: "2.5rem" }}>{activeTab === "pending" ? "🔍" : activeTab === "approved" ? "✅" : activeTab === "published" ? "🚀" : activeTab === "failed" ? "⚠️" : "❌"}</span>
              ...
              {!loading && hasMore && (
                <div style={{ gridColumn: "1/-1", display: "flex", justifyContent: "center", padding: "0.75rem 0" }}>
                  <button onClick={handleLoadMore} disabled={loadingMore}
                    style={{ padding: "0.55rem 2rem", background: "var(--bg2)", border: "1px solid var(--border2)",
                      borderRadius: "var(--radius-sm)", color: "var(--text2)", fontSize: "0.85rem",
                      fontWeight: 600, cursor: loadingMore ? "not-allowed" : "pointer",
                      display: "flex", alignItems: "center", gap: "0.5rem", opacity: loadingMore ? 0.6 : 1 }}>
                    {loadingMore
                      ...
                      <div key={`${ai}-${ri}-${rti}`} style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.3rem 0.5rem", marginBottom: "0.2rem", background: "var(--bg3)", borderRadius: "var(--radius-sm)", flexWrap: "wrap" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 100, height: 5, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
                          <div style={{ height: "100%", borderRadius: 3, width: `${Math.round((rate.remaining / rate.limit) * 100)}%`,
                            background: rate.remaining < rate.limit * 0.1 ? "var(--red)" : rate.remaining < rate.limit * 0.3 ? "var(--amber)" : "var(--green)" }} />
                        </div>
                        <span style={{ fontSize: "0.7rem", minWidth: 75, textAlign: "right",
                          color: rate.remaining < rate.limit * 0.1 ? "var(--red)" : rate.remaining < rate.limit * 0.3 ? "var(--amber)" : "var(--green)" }}>
      ...
      {/* ── Publish modal ── */}
      {publishTarget && (
        <PublishModal product={publishTarget} stores={stores} defaultStoreId={selectedStoreId}
          onClose={() => setPublishTarget(null)} onConfirm={handlePublishConfirm} />
      )}

      {/* ── Reject All confirm ── */}
      {showRejectAllConfirm && (
        <div onClick={() => setShowRejectAllConfirm(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: "1.25rem 1.25rem 1rem", borderBottom: "1px solid var(--border)" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: "1rem", color: "var(--text1)" }}>Reject all {activeTab}?</div>
            </div>
            <div style={{ padding: "1rem 1.25rem", fontSize: "0.85rem", color: "var(--text2)", lineHeight: 1.6 }}>
              <p style={{ margin: 0 }}>Se moverán <strong style={{ color: "var(--text1)" }}>{stats[activeTab]} productos</strong> a <strong style={{ color: "var(--text1)" }}>Rejected</strong>.</p>
              <p style={{ margin: "0.5rem 0 0", fontSize: "0.78rem", color: "var(--text3)" }}>Esta acción no se puede deshacer.</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", padding: "0 1.25rem 1.25rem" }}>
              <button onClick={() => setShowRejectAllConfirm(false)}
      ...
      {/* ── Toasts ── */}
      <div style={{ position: "fixed", bottom: "1.25rem", right: "1.25rem", zIndex: 1000,
        display: "flex", flexDirection: "column", gap: "0.4rem", pointerEvents: "none" }}>
        {toasts.map(t => (
          <div key={t.id} style={{
            padding: "0.6rem 0.95rem", borderRadius: "var(--radius-sm)", fontSize: "0.8rem",
            fontWeight: 500, maxWidth: 340, boxShadow: "0 4px 20px rgba(0,0,0,0.4)", border: "1px solid",
      ...
      <div style={{ textAlign: "center", padding: "0.6rem", borderTop: "1px solid var(--border)", fontSize: "0.72rem", color: "var(--text3)", flexShrink: 0 }}>
        <a href="/privacy" target="_blank" rel="noopener noreferrer" style={{ color: "var(--text3)", textDecoration: "none", marginRight: "0.6rem" }}>Privacy Policy</a>
        · © 2026 DropFlow
      </div>
